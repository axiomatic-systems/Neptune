...
main(int argc, char** argv)
{
    ...
    if (verbose) {
        unsigned int request_latency = (before_body-before_request).ToMillis();
        unsigned int body_load_time  = (after_body-before_body).ToMillis();
        unsigned int total_load_time = (after_body-before_request).ToMillis();
        unsigned int body_throughput = 0;
        if (body_size && body_load_time) {
            body_throughput = ((8.0 * (double)body_size)/1000.0)/((double)body_load_time/1000.0);
        }
        unsigned int total_throughput = 0;
        if (body_size && total_load_time) {
            total_throughput = ((8.0*(double)body_size)/1000.0)/((double)total_load_time/1000.0);
        }
